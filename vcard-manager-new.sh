...
function view_contacts() {
    ...
    while true; do
        ...
        if [ "$show_all_search" = true ]; then
            ...
            local filtered_all_contacts=(${original_all_contacts[@]})
            
            # 过滤联系人
            if [ -n "$all_search_query" ]; then
                filtered_all_contacts=()
                for contact_item in "${original_all_contacts[@]}"; do
                    ...
                    if echo "$contact" | grep -i "$all_search_query" >/dev/null 2>&1; then
                        filtered_all_contacts+=($contact_item)
                    fi
                done
            fi
            
            local total_contacts=${#filtered_all_contacts[@]}
            ...
            while true; do
                ...
                # 显示当前页的联系人
                for ((i=start_idx; i<=end_idx; i++)); do
                    local contact_item=${filtered_all_contacts[$i]}
                    local category=$(echo "$contact_item" | cut -d':' -f1)
                    local contact=$(echo "$contact_item" | cut -d':' -f2)
                ...
                case "$contact_choice" in
                    [mM]) show_main_menu; return ;; 
                    [qQ]) echo -e "${GREEN}谢谢使用！${NC}"; exit 0 ;; 
                    [bB]) show_categories=true; show_all_search=false; continue ;; 
                    [fF]) # 重新搜索
                          read -p "请输入搜索关键词 (直接回车清空搜索): " all_search_query
                          break ;; 
                    [sS]) read -p "请输入每页显示数量: " new_size
                          if [ "$new_size" -gt 0 ] 2>/dev/null; then
                              current_page_size=$new_size
                              # 重新计算分页信息
                              total_contacts=${#filtered_all_contacts[@]}
                ...
                if [ "$contact_choice" -ge 1 ] && [ "$contact_choice" -le $total_contacts ]; then
                    local contact_item=${filtered_all_contacts[$((contact_choice-1))]}
                    local category=$(echo "$contact_item" | cut -d':' -f1)
                    local contact=$(echo "$contact_item" | cut -d':' -f2)
...
function edit_contact() {
    ...
    while true; do
        ...
        if [ "$show_categories" = true ]; then
            # 顶部快捷操作栏
            echo -e "${YELLOW}【快捷操作】: m.返回主菜单 | q.退出程序 | c.取消 | f.搜索联系人 | s.自定义每页显示数量 (当前: $current_page_size)${NC}"
            echo -e "${BLUE}-------------------------------------${NC}"
            ...
            if [ -n "$all_search_query" ]; then
                filtered_all_contacts=()
                for contact_item in "${original_all_contacts[@]}"; do
                    local contact_category=$(echo "$contact_item" | cut -d':' -f1)
                    local contact_name=$(echo "$contact_item" | cut -d':' -f2)
                    if echo "$contact_name" | grep -i "$all_search_query" >/dev/null 2>&1; then
                        filtered_all_contacts+=($contact_item)
                    fi
                done
            fi
            ...
            while true; do
                ...
                echo -e "${BLUE}          关键词: $all_search_query${NC}"
                echo -e "${BLUE}=====================================${NC}"
                echo -e "${YELLOW}【快捷操作】: m.返回主菜单 | b.返回分类列表 | q.退出程序 | c.取消 | f.搜索 | s.自定义每页显示数量 (当前: $current_page_size)${NC}"
                ...
                for ((i=start_idx; i<=end_idx; i++)); do
                    ...
                    local contact_category=$(echo "$contact_item" | cut -d':' -f1)
                    local contact_name=$(echo "$contact_item" | cut -d':' -f2)
                    echo -e "${YELLOW}$((i+1)). ${contact_name} (${contact_category})${NC}"
                done
                ...
                case "$contact_choice" in
                    ...
                    [qQ]) echo -e "${GREEN}谢谢使用！${NC}"; exit 0 ;; 
                    [cC]) echo -e "${GREEN}已取消编辑联系人${NC}"; sleep 1; show_main_menu; return ;; 
                    [bB]) show_categories=true; show_all_search=false; break ;; 
                    [fF]) # 重新搜索
                          read -p "请输入搜索关键词 (直接回车清空搜索): " all_search_query
                          break ;; 
                    [sS]) read -p "请输入每页显示数量: " new_size
                ...
                if [ "$contact_choice" -ge 1 ] && [ "$contact_choice" -le $total_contacts ]; then
                    local contact_item=${filtered_all_contacts[$((contact_choice-1))]}
                    category=$(echo "$contact_item" | cut -d':' -f1)
                    contact=$(echo "$contact_item" | cut -d':' -f2)
                    break
                else
                    echo -e "${RED}无效选择，请重新输入${NC}"; sleep 1; continue
                fi
                ...
                case "$contact_choice" in
                    ...
                    [fF]) # 处理搜索功能
                          ...
                          if [ -n "$search_query" ]; then
                              # 过滤联系人列表，支持模糊搜索
                              filtered_contacts=()
                              for contact_name in "${original_contacts[@]}"; do
                                  if echo "$contact_name" | grep -i "$search_query" >/dev/null 2>&1; then
                                      filtered_contacts+=($contact_name)
                                  fi
                              done
                          else
                              # 清空搜索，恢复原始列表
